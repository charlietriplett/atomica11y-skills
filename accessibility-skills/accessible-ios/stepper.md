---
id: accessibility-skills-ios-stepper
title: "Stepper"
platform: ios
skill_type: component-assessment
version: 1.0.0
generated: "2026-09-13"
source: https://www.atomica11y.com/accessible-ios/stepper/
---

[Source material](https://www.atomica11y.com/accessible-ios/stepper/)

# iOS Stepper skills



## How to test a stepper 

Given that I am on a screen with a stepper

### 1. Keyboard only

- WHEN I use the arrow keys or ctrl + tab to move focus to a stepper button

  - I SEE focus is visually indicated (strongly if high contrast enabled)

- THEN when I use the spacebar to activate the stepper button
 
  - I SEE the state is changed


### 2. VoiceOver screenreader

- WHEN I use a screenreader AND I swipe to focus on a stepper button

  - I HEAR its purpose is clear and matches visible label

  - I HEAR it identifies itself as a button

  - I HEAR it describes what value it affects

  - I HEAR it expresses stepper value

- THEN when I doubletap with the stepper button in focus
 
  - I HEAR the state is changed


### 3. Device settings

- WHEN I use text resize I SEE text can resize up to 200% without losing information

- WHEN I use increased contrast I SEE focus outline becomes high contrast


> AtomicA11y accessibility skills v1.0.0 | Generated 2026-09-13